            ...
            async def _poll():
                ...
                try:
                    ...
                    if not api.la_loi_mang(e):
                        # Máy chủ CÓ trả lời, chỉ là trả lời lỗi (thường 404 "job đã hết hạn")
                        # — job coi như đã mất hẳn, thử lại vô ích (khuôn mẫu cham_ach.py::_poll()).
                        spinner.set_visibility(False)
                        btn_cancel.set_visibility(False)
                    ...
                    ui.notify(
                        'Mất liên lạc với máy chủ khi theo dõi tiến độ — job có thể vẫn đang '
                        'chạy trên máy chủ.', type='negative', timeout=0,
                    )
                    return
