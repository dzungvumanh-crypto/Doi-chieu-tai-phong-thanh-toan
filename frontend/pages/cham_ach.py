            ...
            async def _poll():
                ...
                try:
                    ...
                    if not api.la_loi_mang(e):
                        ...
                        # "job đã hết hạn"). Thử lại thêm vừa vô ích vừa dẫn tới
                        # báo sai "job vẫn đang chạy" ở nhánh dưới.
                        progress_card.set_visibility(False)
                        _stop_running()
                        _append_log(f'[LỖI] Máy chủ từ chối theo dõi tiến trình: {e}')
                        ui.notify(str(e), type='negative', timeout=0, close_button='Đóng')
                        return
                    state['poll_fails'] += 1
            ...
            async def _thuc_hien_chay():
                ...
                try:
                    ...
                    res = await asyncio.to_thread(
                        ...
                        timeout=600.0,   # tới 500 MB, backend ghi hết ra đĩa mới trả lời
                    )
                except Exception as e:
                    spinner.set_visibility(False)
                    progress_card.set_visibility(False)
