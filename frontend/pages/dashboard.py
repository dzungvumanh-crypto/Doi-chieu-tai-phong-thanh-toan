    ...
    with _content_area():
        ...
        with content:
            ...
            with ui.card().classes("w-full p-4 rounded-xl shadow-sm bg-white"):
                ...
                # ── Biểu đồ cột nhóm: 4 phòng, mỗi phòng 2 cột (đúng hạn / muộn) ──
                dept_slots = [
                ...
                by_code = {(row.get("dept_code") or "").upper(): row for row in by_dept}
                labels      = [lbl for _, lbl in dept_slots]
                ontime_vals = [(by_code.get(code) or {}).get("on_time", 0) for code, _ in dept_slots]
                late_vals   = [(by_code.get(code) or {}).get("late", 0)    for code, _ in dept_slots]
                ...
                if not by_dept:
                    ...
                    ui.echart({
                        ...
                        "series": [
                            {"name": "Đúng hạn", "type": "bar", "barGap": "10%", "data": ontime_vals,
                             "itemStyle": {"color": "#16a34a"},
                             "label": {"show": True, "position": "top"}},
                            {"name": "Nộp muộn", "type": "bar", "data": late_vals,
