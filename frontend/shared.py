...
async def open_folder_picker(on_select, *, initial_path: str = "") -> None:
    ...
    Danh sách gốc/ổ đĩa lấy từ /api/fs/browse — fail-closed từ 2026-09-01 (xem
    backend/core/config.py::folder_picker_roots()): chưa cấu hình FOLDER_PICKER_ROOTS
    trong .env thì backend trả lỗi 400 rõ ràng thay vì mặc định liệt kê toàn bộ ổ đĩa,
    dialog này hiện lỗi đó qua error_label như mọi lỗi tải thư mục khác."""
