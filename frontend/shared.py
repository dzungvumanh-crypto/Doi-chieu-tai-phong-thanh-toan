...
# Thư mục vừa dùng — lưu theo từng người đăng nhập (app.storage.user), vì mỗi
# người đối chiếu quen chạy trên một nhóm thư mục cố định.
_RECENT_FOLDERS_KEY = "fs_recent_folders"
...
async def open_folder_picker(on_select, *, initial_path: str = "") -> None:
    ...
    MỘT quy tắc duy nhất, cố ý không có ngoại lệ: **bấm 1 lần vào một dòng = đi
    VÀO thư mục đó**, và nút "Chọn thư mục này" luôn xác nhận **thư mục đang mở**
    (hiển thị ngay cạnh nút). Không có "tô sáng để chọn", không có double-click.

    Lý do bỏ cơ chế cũ (2026-08-12, người dùng báo "bấm vào ổ C không được"):
    cũ dùng click=chọn / double-click=mở, nhưng ở màn hình danh sách ổ đĩa thì
    nút xác nhận bị disable và thao tác chọn KHÔNG mở khoá nó → bấm ổ C xong là
    tắc, chỉ thoát được bằng double-click — thao tác không ai đoán ra trên web.

    Hai lối tắt cho người dùng hằng ngày: ô dán đường dẫn (copy thẳng từ thanh
    địa chỉ Windows Explorer, Enter là đi) và danh sách thư mục vừa dùng gần đây.
    ...
    with dialog, ui.card().classes("p-4").style("min-width: 640px; max-width: 90vw"):
        ...
        # ── Lối tắt 1: dán thẳng đường dẫn ──
        with ui.row().classes("w-full items-center gap-2 mb-2"):
            path_input = ui.input(
                placeholder="Dán đường dẫn vào đây rồi bấm Enter — VD: G:\\ACH check doi chieu 10.08",
            ).props("outlined dense clearable").classes("flex-1")
            btn_goto = ui.button("Đi tới", icon="arrow_forward", color="blue-7").props("dense")

        # ── Lối tắt 2: thư mục vừa dùng ──
        recent_row = ui.row().classes("w-full items-center gap-1 flex-wrap mb-2")
        ...
        with ui.row().classes("w-full items-center gap-1 mb-1"):
            ...
            btn_home = ui.button("Máy tính", icon="computer").props("flat dense no-caps")
        ...
        hint_label = ui.label("Bấm 1 lần vào thư mục để đi vào trong đó.").classes(
            "text-xs text-gray-400 mt-1"
        )
        dang_o_label = ui.label("").classes("text-sm text-gray-700 mt-2 break-all")
        ...
        def _render_recent():
            recent_row.clear()
            recent = app.storage.user.get(_RECENT_FOLDERS_KEY) or []
            if not recent:
                recent_row.set_visibility(False)
                return
            recent_row.set_visibility(True)
            with recent_row:
                ui.label("Gần đây:").classes("text-xs text-gray-500")
                for p in recent:
                    ui.button(p).props("flat dense no-caps size=sm").classes(
                        "text-xs normal-case max-w-[280px] truncate"
                    ).on("click", _crumb_click(p))
        ...
        def _render_list():
            ...
            with list_area:
                ...
                for ent in state["entries"]:
                    row = ui.row().classes(
                        "w-full items-center gap-2 px-3 py-3 cursor-pointer border-b hover:bg-red-50"
                    )
                    with row:
                    ...
                    row.on("click", _crumb_click(ent["path"]))
        ...
        async def _load(path, *, is_initial: bool = False):
            ...
            if state["path"] is None:
                ...
                dang_o_label.set_text("Hãy bấm vào một ổ đĩa để mở.")
                dang_o_label.classes(replace="text-sm text-gray-500 mt-2 italic")
                hint_label.set_text("Bấm 1 lần vào ổ đĩa để mở nó.")
            else:
                btn_up.props(remove="disable")
                btn_select.props(remove="disable")
                dang_o_label.set_text(f"Đang ở: {state['path']}")
                dang_o_label.classes(replace="text-sm text-gray-700 mt-2 break-all")
                hint_label.set_text("Bấm 1 lần vào thư mục để đi vào trong đó.")
                path_input.value = state["path"]

            _render_breadcrumb(res.get("breadcrumbs", []))
            _render_list()

        async def _goto():
            # Windows "Copy as path" bọc đường dẫn trong dấu nháy kép — bỏ đi cho người dùng.
            raw = (path_input.value or "").strip().strip('"')
            if not raw:
                return
            await _load(raw)

        def _luu_gan_day(path: str) -> None:
            recent = [p for p in (app.storage.user.get(_RECENT_FOLDERS_KEY) or []) if p != path]
            app.storage.user[_RECENT_FOLDERS_KEY] = [path] + recent[: _RECENT_FOLDERS_MAX - 1]
        ...
        btn_home.on("click", lambda: _load(None))
        btn_goto.on("click", _goto)
        path_input.on("keydown.enter", _goto)
