...
_MK = "matkhau-test-song-phuong-service"
...
class TestZipCryptoNumbaFastPath:
    ...
    def test_giai_ma_dung_noi_dung_goc(self):
        ...
        with pyzipper.AESZipFile(io.BytesIO(zip_bytes)) as zf:
            zf.setpassword(pwd)
            stream, dung_numba = svc._mo_doc_1_file_thanh_vien(zf, "gl02.csv", pwd)
            with stream:
                out = stream.read()
        assert dung_numba is True
    ...
    def test_sai_mat_khau_raise(self):
        ...
        with pyzipper.AESZipFile(io.BytesIO(zip_bytes)) as zf:
            ...
            with pytest.raises(ValueError, match="Sai mật khẩu"):
                svc._mo_doc_1_file_thanh_vien(zf, "gl02.csv", b"mat-khau-sai")
    ...
    def test_khop_ket_qua_voi_pyzipper_that(self):
        ...
        with pyzipper.AESZipFile(io.BytesIO(zip_bytes)) as zf2:
            zf2.setpassword(pwd)
            stream, _ = svc._mo_doc_1_file_thanh_vien(zf2, "gl02.csv", pwd)
            with stream:
                actual = stream.read()
    ...
    def test_entry_aes_that_khong_bi_nham_thanh_zipcrypto(self):
        ...
        `zf.open()` streaming gốc, KHÔNG được đường tắt ZipCrypto xử lý nhầm — dù `compress_type`
        sau khi pyzipper decode extra field AES cũng trả về 8 (giống hệt ZipCrypto), phải phân
        biệt bằng `wz_aes_version`."""
        pwd = b"aes-pwd"
        buf = io.BytesIO()
        ...
        with pyzipper.AESZipFile(buf) as zf2:
            ...
            zf2.setpassword(pwd)
            stream, dung_numba = svc._mo_doc_1_file_thanh_vien(zf2, "gl02.csv", pwd)
            with stream:
                out = stream.read()
        assert dung_numba is False
        assert out == _GL02_CONTENT
    ...
    def test_khong_co_numba_van_giai_ma_dung(self, monkeypatch):
        ...
        thiếu numba/numpy KHÔNG được làm sập backend, chỉ rơi về `zf.open()` streaming gốc của
        pyzipper (chậm hơn, vẫn đúng). Giả lập thiếu numba bằng `monkeypatch` cờ `_CO_NUMBA`,
        không gỡ thật thư viện — vẫn phải ra đúng kết quả qua đường lui."""
    ...
    def test_process_zip_end_to_end_voi_zipcrypto(self, monkeypatch, tmp_path):
        ...
        kịch bản GL02 thật, không phải fixture AES như các test khác trong dự án. `process_zip()`
        nhận ĐƯỜNG DẪN file (2026-09-01, develop đổi từ nhận bytes sang đọc từ đĩa) — ghi ra file
        tạm trước khi gọi."""
        monkeypatch.setattr(svc, "TEMP_DIR", tmp_path / "_out")
        pwd = _MK.encode()
        zip_bytes = _make_zipcrypto_zip("gl02.csv", _GL02_CONTENT, pwd)
        zip_path = tmp_path / "gl02.zip"
        zip_path.write_bytes(zip_bytes)

        result = svc.process_zip(zip_path)
