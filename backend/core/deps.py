...
def get_current_staff(
    ...
    if not staff_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token không hợp lệ")
    row = db.execute(
        "SELECT * FROM ksnb_staff WHERE id = ? AND is_active = 1", (int(staff_id),)
    ).fetchone()
    if not row:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Tài khoản không tồn tại")
    if get_session_ip(int(staff_id)) is None:
        raise HTTPException(
...
def require_handover_write(
    ...
    if current["role"] in ("chuyen_vien", "pho_phong", "truong_phong") and dept_id:
