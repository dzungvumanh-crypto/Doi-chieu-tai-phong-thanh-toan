...
def enqueue(method: str, path: str, status_code: int,
            auth_header: str, hdr_ip: str | None, client_ip: str | None,
            actor_id: int | None = None, detail: str | None = None,
            noi_dung: str | None = None) -> None:
    """Bỏ một dòng audit vào hàng đợi. Không bao giờ chặn, không bao giờ raise.

    `actor_id`/`detail` để trống thì suy như cũ: người thao tác lấy từ JWT
    trong `auth_header`, mô tả là "HTTP <mã>". Truyền tường minh khi endpoint
    xác thực bằng cách KHÁC JWT — hiện có 2 endpoint buffer của Đối chiếu
    CITAD dùng header `X-Extension-Token`, backend tra ra đúng người nhưng
    `auth_header` rỗng nên nếu không truyền vào đây thì dòng audit mất tên
    người thao tác.

    `noi_dung` là tóm tắt dữ liệu gửi lên (xem `audit_body`), nối sau mã HTTP.
    Nó bị bỏ qua khi `detail` được truyền tường minh — dòng ngữ nghĩa đã tự mô
    tả rõ hơn mọi thứ suy từ body.
    """
    global _da_canh_bao_day
    try:
        _q.put_nowait((method, path, status_code, auth_header, hdr_ip, client_ip,
                       actor_id, detail, noi_dung))
        _da_canh_bao_day = False
    except queue.Full:
...
def _mo_ta(status_code: int, noi_dung: str | None) -> str:
    """Chuỗi `detail` cho dòng do middleware ghi: mã HTTP + tóm tắt nội dung.

    Mã HTTP đứng ĐẦU và giữ đúng dạng "HTTP <mã>" — `audit_labels.describe_result`
    dò mã bằng regex trên chuỗi này, còn `describe_detail` cắt bỏ đúng phần đầu
    để cột Chi tiết không lặp lại thứ cột Kết quả đã nói.
    """
