...
from backend.models import SourceUser, KSNBStaff
...
class _KSNBProxy:
    """Duck-type wrapper quanh KSNBStaff để dùng chung với SourceUser lookup."""
    def __init__(self, staff):
        self.user_code  = staff.ipcas_code or ''
        self.full_name  = staff.payment_username or ''
        self.vn_name    = staff.full_name or ''
...
def _get_month_from_date(date_str: str) -> int:
    """Lấy tháng từ chuỗi ngày dạng dd/mm/yyyy."""
    try:
...
def _get_year_from_date(date_str: str) -> int:
    """Lấy năm từ chuỗi ngày dạng dd/mm/yyyy."""
    try:
        parts = str(date_str).strip().split('/')
        if len(parts) >= 3:
...
def parse_ipcas(file_bytes: bytes) -> tuple:
    """
    Parse file XLS từ IPCAS (all GDV hoặc all HKV).
    Trả về (rows, violations, month, year).
    rows: list[dict] với các key: user_code, tong_gd, gd_hk_dung, chua_hk, bt_huy
    violations: list[dict] — user có name_5 != 0
    """
    df = pd.read_excel(io.BytesIO(file_bytes), engine='xlrd', header=0)
    violations = []
    month, year = 0, 0
    # aggregated[user_code] = {tong_gd, gd_hk_dung, gd_sai, chua_hk, bt_huy}
    aggregated: dict = {}
    ...
    for df_idx, row in df.iterrows():
        ...
        if gd_sai != 0:
            # df_idx là 0-based sau header → dòng Excel = df_idx + 2
...
def _parse_payment(file_bytes: bytes, header_row: int) -> tuple:
    """
    Parse file xlsx Payment. Aggregate theo cột HKV (username).
    Trả về (rows, violations).
    """
    df = pd.read_excel(io.BytesIO(file_bytes), header=header_row)
    # Chỉ giữ dòng có STT là số nguyên dương
...
def parse_payment_teller(file_bytes: bytes) -> tuple:
    """Header teller ở row 6 (0-indexed)."""
    return _parse_payment(file_bytes, header_row=6)


def parse_payment_backchecker(file_bytes: bytes) -> tuple:
    """Header backchecker ở row 10 (0-indexed)."""
    return _parse_payment(file_bytes, header_row=10)


# ── Enrich rows → gắn vn_name + phòng ───────────────────────────────────────

def enrich_and_group(rows: list, db: Session, is_payment: bool = False) -> dict:
    """
    Lookup source_users, gắn vn_name và dept_name vào mỗi row.
    Trả về dict nhóm theo dept_name: {dept_name: [enriched_rows]}.
    Rows không map được vào source_users → nhóm 'Không xác định'.
    """
    ...
    if is_payment:
        users = db.query(SourceUser).options(joinedload(SourceUser.department)).filter(
            func.lower(func.trim(SourceUser.full_name)).in_(lower_codes)
        ).all()
        lookup = {(u.full_name or '').strip().lower(): u for u in users}
            ...
            if code not in lookup:
                for suffix in ('1', '2', '3'):
                    ...
                    if candidate in lookup:
                        lookup[code] = lookup[candidate]
                        break
                if code not in lookup:
                    u = db.query(SourceUser).options(joinedload(SourceUser.department)).filter(
                        func.lower(func.trim(SourceUser.full_name)).like(code + '%')
                    ).first()
                    if u:
                        lookup[code] = u
    else:
        users = db.query(SourceUser).options(joinedload(SourceUser.department)).filter(
            func.lower(func.trim(SourceUser.user_code)).in_(lower_codes)
        ).all()
        lookup = {(u.user_code or '').strip().lower(): u for u in users}

        # Fallback: thử tìm qua KSNBStaff.ipcas_code (nhân viên KSNB)
        missing = [c for c in lower_codes if c not in lookup]
        if missing:
            ksnb_users = db.query(KSNBStaff).options(joinedload(KSNBStaff.department)).filter(
                func.lower(func.trim(KSNBStaff.ipcas_code)).in_(missing),
                KSNBStaff.is_active == True,
            ).all()
            for ksnb in ksnb_users:
                key = (ksnb.ipcas_code or '').strip().lower()
                if key not in lookup:
                    # Bọc KSNBStaff thành duck-type giống SourceUser để dùng chung code bên dưới
                    lookup[key] = _KSNBProxy(ksnb)
    ...
    for row in rows:
        ...
        if user:
            vn_name = user.vn_name or user.full_name or row['user_code']
            user_code_ipcas = user.user_code
...
def merge_hkv(ipcas_rows: list, payment_rows: list, db: Session) -> list:
    """
    Ghép dữ liệu IPCAS HKV và Payment Backchecker theo user.
    IPCAS: lookup source_users.user_code
    Payment: lookup source_users.full_name
    Trả về list merged rows (1 dòng / user) với cột system = 'IPCAS'|'Payment'|'Both'.
    """
    # Build lookup dicts — payment dùng lowercase key để cross-match với DB full_name
    ...
    if ipcas_codes:
        ...
        for u in db.query(SourceUser).options(joinedload(SourceUser.department)).filter(
            func.lower(func.trim(SourceUser.user_code)).in_(lower_ipcas)
        ).all():
            ipcas_users[(u.user_code or '').strip().lower()] = u

    payment_users = {}
    if payment_names:
        lower_payment_names = [n.strip().lower() for n in payment_names]
        for u in db.query(SourceUser).options(joinedload(SourceUser.department)).filter(
            func.lower(func.trim(SourceUser.full_name)).in_(lower_payment_names)
        ).all():
            payment_users[(u.full_name or '').strip().lower()] = u
    ...
    for code, ipcas_row in ipcas_by_code.items():
        ...
        vn_name = (u.vn_name or u.full_name or code) if u else code
        payment_username = u.full_name if u else None
        ...
        seen_ipcas.add((u.user_code or code).lower() if u else code.lower())

    # Thêm các user chỉ có trong Payment (không có trong IPCAS)
    # username đã là lowercase key từ payment_by_name
    for username, payment_row in payment_by_name.items():
        u = payment_users.get(username)  # payment_users cũng keyed lowercase
        if u and (u.user_code or '').lower() in seen_ipcas:
            continue
        vn_name = (u.vn_name or u.full_name or username) if u else username
        merged.append({
            'user_code': u.user_code if u else username,
            'vn_name': vn_name,
            'ipcas_tong_gd': 0,
            'ipcas_gd_hk_dung': 0,
            'ipcas_gd_sai': 0,
...
def _save_row_styles(ws, row_num: int) -> tuple:
    """Lưu style (font, alignment, border, fill) của từng cell trong 1 hàng."""
...
def _apply_row_styles(ws, row_num: int, styles: list, height):
    """Apply lại style đã lưu vào hàng mới — không ghi value."""
    from copy import copy
...
def generate_dept_excel(dept_name: str, month: int, year: int,
    ...
    """Sinh file Excel báo cáo tổng hợp hậu kiểm — dùng template, chỉ thay nội dung."""
    template_path = _TEMPLATES_DIR / 'Báo cáo Tổng hợp hậu kiểm theo phòng.xlsx'
    wb = openpyxl.load_workbook(str(template_path))
    ws = wb.active
    ws.title = f'T{month:02d}.{year}'

    # ── Cập nhật tiêu đề (chỉ set value, giữ nguyên font/format của template) ──
    ws['A5'].value = f'BÁO CÁO TỔNG HỢP HẬU KIỂM PHÒNG {dept_name.upper()}'
    ws['A6'].value = f'Tháng {month:02d} năm {year}'

    # ── Lưu style từ các hàng mẫu TRƯỚC KHI XÓA ──
    # Template: R8=section hdr, R9=data, R16=total, R17=section II hdr, R26=sign label, R30=name
    ...
    # ── Xóa merge cells và data rows từ R8 trở đi ──
    ...
    # ── Helper: ghi dòng data (chỉ value) ──
    ...
    # ── Helper: ghi dòng tổng (chỉ value + formula) ──
...
def _replace_date_runs(para, month: int, year: int):
    """
    Thay 'tháng MM/YYYY', 'Tháng MM/YYYY', 'THÁNG MM NĂM YYYY' trong paragraph.
    Text bị tách nhiều run → dùng span map để merge đúng chỗ, giữ format xung quanh.
    """
    import re as _re
    runs = para.runs
    if not runs:
    ...
    # Thu thập tất cả match, xử lý từ phải sang trái để giữ vị trí ổn định
    all_matches = []
    ...
    # Xây dựng bản đồ vị trí từng run trong full_text
    pos = 0
    spans = []
    for r in runs:
    ...
    for match_start, match_end, replacement in all_matches:
        ...
        # Rebuild spans sau mỗi lần thay
        pos = 0
        spans = []
        full_text = ''.join(r.text for r in runs)
        for r in runs:
...
def _set_row_cells(row, values):
    """Ghi text vào từng ô duy nhất (bỏ qua ô merged trùng lặp)."""
...
def generate_center_word(month: int, year: int,
    ...
    """
    Sinh Word báo cáo hậu kiểm trung tâm — dùng template.
    hkv_ipcas/hkv_payment: list merged rows từ merge_hkv()
    dept_data: list[{dept_name, ipcas_tong_gd, ..., payment_tong_gd, ...}]
    """
    import copy
    template_path = _TEMPLATES_DIR / 'Báo cáo hậu kiểm tháng.docx'
    doc = Document(str(template_path))

    # ── Thay month/year trong tất cả paragraph (P2, P7, P9, P11, P23...) ──
    for para in doc.paragraphs:
        _replace_date_runs(para, month, year)

    # ── Cập nhật ngày tháng — helper xóa runs thừa ──
    def _set_date_cell(cell, text: str):
        para = cell.paragraphs[-1] if cell.paragraphs else None
        if para is None:
            return
        # Tìm para có text ngày (có thể là para cuối cùng của cell đa-para)
    ...
    # ── Fill Table 1 (HKV data, 22 rows) ──
    t1 = doc.tables[1]
    # Lưu XML template các kiểu hàng trước khi xóa
    ...
    # Tổng (I) — col 2+3 merged → 8 unique cells, không có '' thừa ở giữa
    ...
    # II. section header (clone giữ nguyên merge + format)
    ...
    # ── Fill Table 4 (dept summary, 14 rows) ──
    t4 = doc.tables[4]
    sec_hdr_xml  = copy.deepcopy(t4.rows[1]._tr)   # section header (merged)
    data_r4_xml  = copy.deepcopy(t4.rows[2]._tr)   # data row
    total_r4_xml = copy.deepcopy(t4.rows[6]._tr)   # Tổng (I)
    grand_r4_xml = copy.deepcopy(t4.rows[13]._tr)  # Tổng (I + II)

    # Xóa rows từ 1 đến cuối
    while len(t4.rows) > 1:
    ...
    def _dept_label(name: str) -> str:
        """Bỏ tiền tố 'Phòng ' để hiển thị gọn trong cột Phòng nghiệp vụ."""
...
def build_zip(files: dict) -> bytes:
    """Đóng gói dict {filename: bytes} thành 1 file ZIP in-memory."""
    buf = io.BytesIO()
